...
class ISRDataset(Dataset):
    ...
    def __init__(self, data_path, scale=2, img_size=32, num_patchs=64):
        self.img_size = img_size
        self.scale = scale
        self.num_patchs = num_patchs
        self.image_paths = self._load_data(data_path)

    def _load_data(self, data_path):
        image_paths = glob.glob(os.path.join(data_path, 'X', '*'))
        if self.scale == 2:
            image2_paths = glob.glob(os.path.join(data_path, 'X2', '*'))
            image_paths.extend(image2_paths)
        return image_paths
    ...
    def __getitem__(self, item):
        ...
        if self.scale == 2:
            if 'X2' in image_path:
                low_image_path = image_path.replace('X2', 'X4').replace('x2', 'x4')
            else:
                low_image_path = image_path.replace('X', 'X{}'.format(self.scale)). \
                    replace('.png', 'x{}.png'.format(self.scale))
        else:
            low_image_path = image_path.replace('X', 'X{}'.format(self.scale)). \
                replace('.png', 'x{}.png'.format(self.scale))
        low_img = cv2.imread(low_image_path)
